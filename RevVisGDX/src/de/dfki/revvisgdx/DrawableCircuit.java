...
public class DrawableCircuit implements Drawable {
	...
	public DrawableCircuit(Biochip toDraw) {
		this.data = toDraw;
		field = new DrawableSprite("assets/textures/GridMarker.png", 1, 1);
		field.color = new Color(0.8f, 0.9f, 1f, 1f);
		blob = new DrawableSprite("assets/textures/Blob.png", 1, 1);
	...
	private void drawGates() {
		...
		if (autoAdvance) {
			...
			if (lastAutoStepAt + (long)((1f / this.autoSpeed) * 1000) < current) {
				...
				currentTime++;
				ClassicUIContainer.singleton.time.setValue((int) currentTime);
			}
